...
- Все исходные изображение размещаются в папке `app/img/_src/`
  Все исходные изображения подготавливаются в разрешении 2x, а уже в процессе работы Gulp автоматически сжимаются и делятся на 2 группы, выгружаясь в папки `app/img/@1x` и `app/img/@2x/`. Именно такая структура изображений проекта позволяет контролировать изменённые изображения и включать обработку только в том случае, если файл был добавлен или изменён. Именно поэтому изображения разбиваются по папкам без изменения оригинальных имён.
